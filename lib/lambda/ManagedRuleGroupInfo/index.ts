...
export async function handler(Event: SNSEvent){
  ...
  for(const record of Event.Records){
    ...
    switch(MESSENGER){
      case "Slack":
        await mangedRuleGroupNotificationSlack(CurrentDefaultVersion, WAFRULEGROUPINFOTEXT, record);
        break;
      case "Teams":
        await mangedRuleGroupNotificationTeams(CurrentDefaultVersion, WAFRULEGROUPINFOTEXT, record);
        break;
    }
  }
}